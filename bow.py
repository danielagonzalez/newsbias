	...
	for idx, source in enumerate (SOURCES):
		with open('data/' + source + '.json', "r") as read_file:
			...
			for article in articles:
				if article['desc']:
					data.append (article['desc'].encode("ascii", "ignore").lower())
					labels.append (label)
...
def train_cnn_lstm (x_train, x_test, y_train, y_test):
	...
	model = Model(sequence_input, predictions)
	model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
	model.fit(x_train, y_train, validation_split=0.1, nb_epoch=3, batch_size=128)
	output_test = model.predict(x_test)
	print("test auc:", roc_auc_score(y_test,output_test[:,1]))

data, labels = use_existing_data ()
x_train, x_test, y_train, y_test = preprocess (data, labels)
train_cnn_lstm (x_train, x_test, y_train, y_test)